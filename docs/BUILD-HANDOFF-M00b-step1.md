...
| Count | Value |
|---|---|
| Player-visible strings on the sheet | **512** |
| of which branch-only (shown on some routes) | 190 |
| by source | content 234 · core 69 (log lines, badges) · app 209 (buttons, headings, hints, badges, tooltips, compositions, messages) |
...
- **The debrief prints raw fact ids as notes.** Rows such as `Notes: g8-sea-exhaustion-limited` and `Notes: g8-corps-division-deepened` are what the person cards in the debrief actually show — the ledger's note ids, not their labels. That is an app defect the sheet made visible; it belongs to the M00b presentation pass (render the fact labels), not to Codex.
